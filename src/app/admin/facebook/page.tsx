...
export default function ConnectFacebookPage() {
  ...
  const loadStatus = async () => {
    try {
      ...
      const j = await readJson<{ connected?: boolean; pageName: string | null; pageId: string | null }>(res, 'Facebook status')
      if (j.connected) setConnected({ pageName: j.pageName, pageId: j.pageId })
  ...
  const submit = async () => {
    ...
    try {
      ...
      const j = await readJson<{ pageName: string | null; pageId: string | null }>(res, 'Facebook connect')
