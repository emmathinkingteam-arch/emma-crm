...
import { Loader2, CheckCircle2, Search, X, UploadCloud, Sparkles } from 'lucide-react'
...
// What the AI pulled off the slip (shape mirrors /api/accounts/read-slip)
...
export default function AddExpenseAutoPage() {
    ...
    const [aiInfo, setAiInfo] = useState<{ confidence: string; note: string } | null>(null)
    ...
    async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        if (!file) return
        setSlipFile(file); setUploadedUrl(null); setUploadedFileId(null)
        setUploadError(null); setAiInfo(null); setUploading(true)
        const fd = new FormData()
        fd.append('file', file)
        try {
            // Stores the slip in Backblaze AND reads it with Claude in one call.
            const res = await fetch('/api/accounts/read-slip', { method: 'POST', body: fd })
            const json = await res.json()
            if (!res.ok || !json.ok) throw new Error(json.error || 'Upload failed')
            setUploadedUrl(json.url)
            setUploadedFileId(json.key)
            if (json.extracted) {
                applyExtracted(json.extracted as ExtractedSlip)
            } else {
                setAiInfo({ confidence: '', note: 'Slip saved, but I couldn’t read it automatically — please fill the fields below.' })
            }
        } catch (err: any) {
    ...
    // Pre-fill the form from what the AI read off the slip. Everything stays
    // editable — this only saves typing; the accountant reviews and hits Save.
    function applyExtracted(x: ExtractedSlip) {
        const bankValid = (id: string) => !!id && banks.some((b) => b.id === id)
        const catValid = (id: string) => !!id && cats.some((c) => c.id === id)

        if (x.date) setDate(x.date)
        if (x.amount > 0) setAmount(String(x.amount))
        const desc = (x.description || '').trim()

        if (x.direction === 'transfer') {
            setMode('transfer')
            if (bankValid(x.bank_id)) setBankId(x.bank_id)
            if (bankValid(x.to_bank_id)) setTransferToBank(x.to_bank_id)
            if (desc) setDescription(desc)
        } else {
            setMode('expense')
            if (catValid(x.category_id)) {
        ...
        let note = ''
        if (x.direction === 'income') {
            note = 'This looks like an INCOME slip (a customer paid us). This form only records expenses & transfers — record income from the Income page. Fields are filled for reference.'
        } else if (x.direction === 'unknown') {
            note = 'I couldn’t tell if this is money in or out — please check the fields.'
        } else if (!bankValid(x.bank_id)) {
            note = 'I couldn’t match the bank account — please pick it.'
        }
        setAiInfo({ confidence: x.confidence || '', note })
    }
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                <div className="mb-5">
                    <h2 className="text-base font-bold text-gray-800 flex items-center gap-1.5">
                        <Sparkles size={15} className="text-violet-500" /> Add Expense — Auto
                    </h2>
                    <p className="text-[11px] text-gray-400 font-medium mt-0.5">
                        Upload the slip first — AI reads it and fills the form. Review, then Save.
                    </p>
                </div>
                ...
                {/* Slip upload — FIRST, the AI fills the rest */}
                <Field label="Slip (photo or PDF) — upload first">
                    <input ref={fileInputRef} type="file" accept="image/*,application/pdf" className="hidden" onChange={handleFileChange} />
                    {!slipFile && !uploadedUrl && (
                        <button type="button" onClick={() => fileInputRef.current?.click()}
                            className="w-full flex flex-col items-center gap-1.5 bg-violet-50 border-2 border-dashed border-violet-200 rounded-xl px-3 py-6 text-sm text-violet-500 hover:border-violet-400 hover:text-violet-600 transition-colors">
                            <UploadCloud size={22} />
                            <span className="font-bold">Upload the slip — AI fills the form</span>
                            <span className="text-[10px] text-violet-400">BOC / ComBank slips, e-receipts, bills…</span>
                        </button>
                    )}
                    {uploading && (
                        <div className="flex items-center gap-2 text-sm text-gray-500 bg-violet-50 border border-violet-200 rounded-xl px-3 py-3">
                            <Loader2 size={14} className="animate-spin text-violet-500" /> Uploading &amp; reading the slip…
                        </div>
                    )}
                    {uploadedUrl && slipFile && (
                        <div className="flex items-center justify-between bg-emerald-50 border border-emerald-200 rounded-xl px-3 py-2.5">
                            ...
                            <button onClick={() => { setSlipFile(null); setUploadedUrl(null); setUploadedFileId(null); setAiInfo(null); if (fileInputRef.current) fileInputRef.current.value = '' }}
                ...
                {aiInfo && (
                    <div className="mb-5 rounded-xl border border-violet-200 bg-violet-50 px-3 py-2.5">
                        <div className="flex items-center gap-2 text-xs font-bold text-violet-700">
                            <Sparkles size={13} /> Auto-filled from the slip
                            {aiInfo.confidence && (
                                <span className={`ml-auto px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${aiInfo.confidence === 'high' ? 'bg-emerald-100 text-emerald-700' :
                                    aiInfo.confidence === 'medium' ? 'bg-amber-100 text-amber-700' : 'bg-rose-100 text-rose-700'
                                    }`}>{aiInfo.confidence} confidence</span>
                            )}
                        </div>
                        <p className="text-[11px] text-violet-500 mt-1">Check the fields below, then Save.</p>
                        {aiInfo.note && (
                            <p className="text-[11px] font-semibold text-amber-600 mt-1.5">{aiInfo.note}</p>
                        )}
                    </div>
                )}
