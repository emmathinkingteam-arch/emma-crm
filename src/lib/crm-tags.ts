...
export interface CrmTagDef {
    ...
    btnOn: string
    /** negative outcome → asks for a reason + lands in admin "Rejected CRM" */
    negative: boolean
}

// NOTE: class strings must stay literal so Tailwind's scanner keeps them.
export const CRM_TAGS: CrmTagDef[] = [
    { key: 'package_sent', label: 'Package details sent', chip: 'bg-blue-50 text-blue-600 border border-blue-100', btn: 'bg-blue-50 text-blue-600 border-blue-100', btnOn: 'bg-blue-600 text-white border-blue-600', negative: false },
    { key: 'payment_sent', label: 'Payment details sent', chip: 'bg-indigo-50 text-indigo-600 border border-indigo-100', btn: 'bg-indigo-50 text-indigo-600 border-indigo-100', btnOn: 'bg-indigo-600 text-white border-indigo-600', negative: false },
    { key: 'bank_sent', label: 'Bank details sent', chip: 'bg-green-50 text-green-600 border border-green-100', btn: 'bg-green-50 text-green-600 border-green-100', btnOn: 'bg-green-600 text-white border-green-600', negative: false },
    { key: 'call_back', label: 'Call back later', chip: 'bg-purple-50 text-purple-600 border border-purple-100', btn: 'bg-purple-50 text-purple-600 border-purple-100', btnOn: 'bg-purple-600 text-white border-purple-600', negative: false },
    ...
    { key: 'fake', label: 'Fake', chip: 'bg-gray-100 text-gray-600 border border-gray-200', btn: 'bg-gray-100 text-gray-600 border-gray-200', btnOn: 'bg-gray-700 text-white border-gray-700', negative: true },
]
