...
//
// Posts exist in two languages: Sinhala (default) and English. Pass lang=en to
// store the English artwork under orders.post_image_url_en instead.
// ============================================================================
...
export async function POST(req: NextRequest) {
  ...
  // Posts come in two languages. The English artwork gets its own column so it
  // never overwrites the Sinhala one — both can exist for the same order.
  const isEnglish = ((formData.get('lang') as string | null) || '').trim() === 'en'
  ...
  const safeCode = ((code || 'post') + (isEnglish ? '-en' : '')).replace(/[^a-zA-Z0-9._-]/g, '-').slice(0, 60)
  ...
  const { error: dbErr } = await sa
    .from('orders')
    .update(isEnglish ? { post_image_url_en: url } : { post_image_url: url })
    .eq('id', orderId)
