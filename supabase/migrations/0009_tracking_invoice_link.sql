-- ─────────────────────────────────────────────────────────────────────────
-- Tracking page: let the customer open/download their invoice. The public
-- invoice page already lives at /invoice/<order_id> (RLS public_invoice_read),
-- so the tracking RPC just needs to expose the order id + whether an invoice
-- was generated.
-- ─────────────────────────────────────────────────────────────────────────
...
begin
  ...
  return jsonb_build_object(
    'found',             true,
    'order_id',          v_order.id,
    'has_invoice',       (v_order.invoice_html is not null),
    'customer_name',     v_customer.name,
    'customer_title',    v_customer.title,
